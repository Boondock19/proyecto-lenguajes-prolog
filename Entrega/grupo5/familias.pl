 ...
 *  de 100 aÃ±os de soledad.
 *
 *  @author Ana Sofia Santos C  17-10602 (Prolog code y PlDoc markup)
 *  @author Jose Matias Gonzalez V 15-10627 (Prolog code y PlDoc markup)
 *  @version ch1.0
...
%% cuñado(+NombreCuÃ±ado:atomo,NombreY:atomo) is nondet
%
%  Predicado que indica el nombre del cuñado de un personaje y
%  el nombre del personaje del que es cuñado.
%  Se lee de manera cuñado(X,Y) donde X es un personaje
%  masculino y puede ser el esposo del hermano o hermana de Y.
%  O X puede ser el hermano del esposo o esposa de Y.
%
%  @param NombreCuñado Nombre del personaje que representa al cuñado de
%  Y.
%  @param NombreY Nombre del personaje que representa de quien es
%  cuÃ±ado X.

cuñado(X,Y) :- personaje(X,masc),
    ((esposo(X,Z),(hermano(Z,Y);hermana(Z,Y)));
    (hermano(X,Z),(esposo(Z,Y);esposa(Z,Y)))).

%% cuñada(+NombreCuñada:atomo,NombreY:atomo) is nondet
%
%  Predicado que indica el nombre de la cuñada de un personaje y
%  el nombre del personaje del que es cuñada.
%  Se lee de manera cuñada(X,Y) donde X es un personaje
%  femenino y puede ser la esposa del hermano o hermana de Y.
%  O X puede ser la hermana del esposo o esposa de Y.
%
%  @param NombreCuñada Nombre del personaje que representa a la cuñada
%  de Y.
%  @param NombreY Nombre del personaje que representa de quien es
%  cuñada X.


cuñada(X,Y) :- personaje(X,fem),
